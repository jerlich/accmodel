function [H hessdata Bnd] = run_quadfit5(x_bf, mydata, ratname, offset)
% function [hessdata Bnd] = run_quadfit5(x_bf, mydata, [ratname], [offset])
% 11-param version
...
default_param = [0 1 1 0 20 1 0.1 0 0.01 1 0.1];
default_param(1:3) = x_bf(1:3);
default_param(5:end) = x_bf(4:end);
do_param = [1 1 1 0 0 1 1 1 1 1 1];

x_peak = x_bf([1:3 5:10]); x_peak = x_peak(:); % ignore B
N = 9; % number of dimensions
cardinal_steps = [0.2 0.5 3 0.05 0.001 0.03 0.01 0.05 0.001];
at_ori = 1;


myfunction = @(x)ll_model_wrapper2(x, mydata, ...
...
fprintf('\n\n');
fprintf('run_quadfit5.m: Now computing quadfits, first evaluating in cardinal coordinates, n = %i dimensions\n', N);
...
fprintf('\n\n');
fprintf('run_quadfit5.m: Eigenvalues of estimate of H based on cardinal steps only:\n');
display(sort(eE));
fprintf('H = \n');
display(H);
fprintf('inv(H) = \n');
...
while n>=1 && (max(abs(eE))/min(abs(eE))>100 || any(eE<0)),
    ...
    fprintf('\nrun_quadfit4.m: finished evaluations at %i dimensions, with %i function evals...\n', n, size(xs,2));
    fprintf('eV = \n');
    display(eV);
    fprintf('eE = \n');
    display(eE);
...
if any(bestE < 0)
    fprintf('run_quadfit5.m: We still estimate at least one negative eigenvalue!\n');
    display(sort(bestE));
...
fprintf('\n\n');
fprintf('run_quadfit5.m: Eigenvalues of estimate of H based on iterative approach:\n');
display(sort(bestE));
...
do_param_linescan = [1 1 1 0 1 1 1 1 1 1 1];
fprintf(1, '\njob_quadfit.m: Doing linescan over B\n');
for i = 1:numel(B),
    xx = x_bf;
    xx(4) = B(i);
    [LL_B(i) LLgrad_B(i,:)] = ll_model_wrapper2(xx, mydata, 'for_fmin', 1, 'do_param', do_param_linescan);
end;
