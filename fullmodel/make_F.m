function [F dFds dFdsig2 dFdB dFdg dFdphi] = make_F(bo, sigma2, gamma, phi, varargin)

pairs = { ...
    'c'         []  ; ... % used only when gamma=1 (lambda = 0)
    }; 
...
for j = 2:numel(x)-1,
    ...
    if abs(gamma-1) < eps && ~isempty(c),
        mu = x(j) + c;
    else
        mu = gamma*x(j) - phi*(1-gamma); 
    end;
...
% The derivative of F with respect to gamma and phi both follow from the
% derivative w.r.t. s
if abs(gamma-1)<eps,
    dFdg = 0;
    dFdphi = 0;
else
    dFdg   = dFds.*(ones(numel(x),1)*x + phi);
    dFdphi = dFds*(gamma-1);
end;