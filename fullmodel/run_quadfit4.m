function [H hessdata Bnd] = run_quadfit4(x_bf, mydata, ratname, offset)
% function [hessdata Bnd] = run_quadfit4(x_bf, mydata, [ratname], [offset])
...
cardinal_steps = [0.2 0.5 3 0.05 0.001 0.03 0.01];
at_ori = 1;
...
fprintf('\n\n');
fprintf('run_quadfit4.m: Now computing quadfits, first evaluating in cardinal coordinates, n = %i dimensions\n', N);
...
fprintf('\n\n');
fprintf('run_quadfit4.m: Eigenvalues of estimate of H based on cardinal steps only:\n');
display(sort(eE));
fprintf('H = \n');
display(H);
fprintf('inv(H) = \n');
...
if any(bestE < 0)
    fprintf('run_quadfit4.m: We still estimate at least one negative eigenvalue!\n');
    display(sort(bestE));
...
fprintf('\n\n');
fprintf('run_quadfit4.m: Eigenvalues of estimate of H based on iterative approach:\n');
display(sort(bestE));
