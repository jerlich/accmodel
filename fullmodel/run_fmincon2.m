...
function [history f hessian fmincon_exit] = run_fmincon2(data, x_init, varargin)
    
pairs = { 
    'dx'                   0.25  ; ...
    'dt'                   0.02  ; ...
    'lower_bound'          [-5     0       0       0       4       0.01    0.005   -5      0   0.01    0.005]; ...
    'upper_bound'          [+5     200     200     30      32      2.0     0.9     +5      1   2.0     0.9]; ...
...
x_names = {'lambda'  'sigma2_a'  'sigma2_s'  'sigma2_i'  'B'  'phi'  'tau_phi'  'bias'  'inatt'  'psi'  'tau_psi'};
...
fprintf(1, 'run_fmincon2.m: x_init = ');
...
if no_hessian,
   [x_fmincon, f, exitflag, output, ~, grad] = ...
      fmincon(@(x)ll_model_wrapper2(x, data, 'dx', dx, 'dt', dt, 'track_history', 1, ...
