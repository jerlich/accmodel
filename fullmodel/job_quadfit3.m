function job_quadfit3(varargin)

ratname = varargin{1};
ntrials = str2double(varargin{2});
offset  = str2double(varargin{3});

fprintf(1, 'job_quadfit3.m: running\n');
...
if ~isempty(best_fits),
    ...
    if rati > numel(all_chrono_rats),
        fprintf('job_quadfit3.m: could not find fits for rat %s, aborting.\n', ratname);
        return;
    else
...
fprintf('job_quadfit3.m: evaluating %s around best fit parameters:\n', ratname);
display(x_bf);

[H hessdata Bnd] = run_quadfit5(x_bf, mydata, ratname, offset);

fprintf('job_quadfit3.m: DONE!\n', ratname);
