function musc_model_fit_wrapper2
...
if ~exist('chrono_fof_rawdata.mat','file')
    [sessid,infside]=bdat lla('select sessid,sign(dose) from pbups.pbupssumm where expgroup="fof"');
    iside(infside==-1)='l';
    iside(infside==1)='r';
...
cfit = load('fmincon_out_fofbi_off0_1809trials');
...
if ~exist('fmincon_out_fof_lapse2.mat','file')
    
    do_param=zeros(1,12);
    do_param(12)=1;
    job_fmincon35musc('fof',x_init,do_param,'lapse2');
    if do_cntrl
        job_fmincon35musc('cntrl',x_init,do_param,'lapse2');
    end
    
end


if ~exist('fmincon_out_fof_gain2.mat','file')
    
    do_param=zeros(1,12);
    do_param(11)=1;
    job_fmincon35musc('fof',x_init,do_param,'gain2');
    if do_cntrl
        job_fmincon35musc('cntrl',x_init,do_param,'gain2');
    end
end
...
if ~exist('fmincon_out_fof_noise2.mat','file')
    
    do_param=zeros(1,12);
    do_param(10)=1;
    job_fmincon35musc('fof',x_init,do_param,'noise2');
    if do_cntrl
        job_fmincon35musc('cntrl',x_init,do_param,'noise2');
    end
end



if ~exist('fmincon_out_fof_bias2.mat','file')
    do_param=zeros(1,12);
    do_param(8)=1;
    job_fmincon35musc('fof',x_init,do_param,'bias2');
    if do_cntrl
        job_fmincon35musc('cntrl',x_init,do_param,'bias2');
    end
end
%%

names={'bias2' 'noise2' 'gain2' 'lapse2'};
clear ll param;
