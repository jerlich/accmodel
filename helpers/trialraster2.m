function [ys, xs] = trialraster2(ref, trialidx, trialvals, pre, post,prewin,postwin)
...
for i = 1:numel(ref),
    ...
    if isempty(trials),
		...
		try
	        ys(i,max_pre+trials(1)+1:trials(end)+max_pre+1) = vals(:)'; 
		
		catch
			ys(i,:) = NaN * ones(size(xs));
            fprintf(2,'Probably something wrong in trialraster2\n');
		keyboard
        end;
    end
end